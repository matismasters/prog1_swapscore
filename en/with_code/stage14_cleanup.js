...
Refactor and clean up the code to send for review.
...
function game() {
  ...
  let players = [];
  do {
    let playerName = prompt("Enter the name of a player or 'next' if you have entered all: ");
  ...
  let numberOfNumbers = askForValidNumber("Enter the number of numbers in the lists: ");
  ...
  while (numberOfRounds > 0) {
    ...
    if (sortedLists.length > 0) {
      console.log(`${sortedLists.length} list${sortedLists.length > 1 ? 's' : ''} sorted!`);
